...
public class Listener implements ITestListener, ISuiteListener{
	...
	public void onTestStart(ITestResult result) {
		...
		ExtendReport.addAuthors(result.getMethod().getConstructorOrMethod().getMethod().getAnnotation(FrameworkAnnotation.class)
			.author());
		ExtendReport.addCategories(result.getMethod().getConstructorOrMethod().getMethod().getAnnotation(FrameworkAnnotation.class)
			.category());
	}
	...
	public void onTestFailure(ITestResult result) {
		FrameworkeLogger.log(LogType.FAIL,result.getMethod().getMethodName() +" is failed");
		FrameworkeLogger.log(LogType.FAIL,result.getThrowable().toString());
        ...
        List<ITestResult> list = exceptionGroup.computeIfAbsent(exceptionType, k-> new ArrayList<>());
        list.add(result);
	}
	...
	public void onTestSkipped(ITestResult result) {
		...
		FrameworkeLogger.log(LogType.SKIP,result.getMethod().getMethodName() +" is skipped");
    ...
    private String createExceptionGroupsTable(Map<String, List<ITestResult>> exceptionGroups) {
        ...
        for (Map.Entry<String, List<ITestResult>> entry : exceptionGroups.entrySet()) {
            tableHtml.append("<tr><td>").append(entry.getKey()).append("</td><td>").append(getFailedTestCases(entry.getValue())).append("</td></tr>");
        }
    ...
    private String getFailedTestCases(List<ITestResult> results) {
        ...
        for (ITestResult result : results) {
            testCases.append(result.getMethod().getMethodName()).append("<br>");
        }
